"""
Experiment 3: T5-based Multi-Label Classification for Polarization Type Detection


Strategies:
  - t5_base        : google-t5/t5-base fine-tuned on the training split
  - t5_large       : google-t5/t5-large fine-tuned on the training split
"""
...
try:
    ...
    print("transformers datasets torch accelerate")
...
def build_hf_dataset(df: pd.DataFrame, tokenizer, max_input_length=256, max_target_length=64):
    ...
    for _, row in df.iterrows():
        ...
        records.append({
            "input_text":  f"classify polarization: {row['text']}",
...
class T5PolarizationClassifier:
    """
    Fine-tunes a T5 model on polarization type classification
    and provides a predict interface identical to llmpolarclassf.
    """

    def __init__(
        self,
        model_name: str = "google/flan-t5-base",
        output_dir: str = "./t5_checkpoints",
    ...
    def predict(self, text: str) -> np.ndarray:
        """Predict a single text -> binary label array of shape (5,)"""
        if not self._is_fitted:
            raise RuntimeError("Call fit() before predict().")

        input_text = f"classify polarization: {text}"
    ...
    def predict_batch(self, texts: list, verbose: bool = True) -> np.ndarray:
        """Predict a list of texts -> array of shape (N, 5)"""
        predictions = []
...
def run_t5_experiment(
    ...
    results_dir: str = "./results_subtask2",
    **trainer_kwargs,             
) -> dict:
    """
    Fine-tune and evaluate one or more T5 variants

    Returns a dict  { model_key: { 'results': ..., 'predictions': ... } }
    """
    ...
    for key in model_keys:
        ...
        print(f"\n{'='*70}")
        print(f"T5 Experiment: {key}  ({model_name})")
        print(f"{'='*70}\n")

        try:
            classifier = T5PolarizationClassifier(
    ...
    if all_results:
        ...
        cmp_df.to_csv(os.path.join(results_dir, "t5_model_comparison.csv"), index=False)
...
def _technique_description(key: str, model_name: str) -> str:
    base_desc = f"""
This experiment fine-tunes **{model_name}** for multi-label polarization type classification
using a **text-to-text (Seq2Seq)** framing.

**Input format:**
```
classify polarization: <raw_text>
```

**Target format:**
```
political, racial/ethnic    (comma-separated active labels, or "none")
```
...
**Why text-to-text?**
T5 treats every NLP task as string-to-string generation.  This lets the model
leverage its pre-training signal and handle multi-label output naturally —
instead of N binary heads, a single decoder generates the full label set.
"""
    if "flan" in key:
        base_desc += """
**Flan-T5 advantage:**
Flan-T5 was instruction-tuned on hundreds of tasks, so it already understands
classification-style instructions.  This gives it a head-start over vanilla T5,
...
**Notes:**
- Text-to-text framing handles multi-label output without threshold tuning.
- Flan-T5 variants tend to outperform vanilla T5 on low-resource classification
  tasks due to their instruction fine-tuning pre-training.
- Larger models (large) trade inference speed for accuracy gains.
"""
...
def main():
    ...
    print("T5-BASED POLARIZATION TYPE CLASSIFICATION")
    ...
    if not TRANSFORMERS_AVAILABLE:
        print("transformers not installed – cannot run experiment.")
        return


    model_keys = ["flan_t5_base", "t5_base"]   # quickest to validate
